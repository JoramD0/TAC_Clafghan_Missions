// TAC Settings
force tac_apollo_enabled = true;			// Set to 'false' to disable persistence for missions where we aren't using our own gear.
force tac_insignia_enabled = false;			// Disables insignia, which should fix Theseus Pants.

// ACE Settings
force ace_map_defaultChannel = 1;			// Sets default channel when joining to "Side".
force ace_weather_windSimulation = false;	// Wind based on the maps geographical location. Enabling this will override the settings you've set in EDEN.
force ace_nightvision_effectScaling = 0.1;	// Blur, Grain and Brightness effects for Nightvision.

force ace_map_BFT_Enabled = true;
force ace_map_BFT_Interval = 30;
force ace_map_BFT_HideAiGroups = true;